...
const ReportTable = ({ cookieStands, deleteStand }) => {
  ...
  return cookieStands.length > 0 ? (
    <table className="w-2/3 mx-auto my-4 border-collapse">
      <thead>
        <tr>
          <th className="border border-emerald-600 bg-emerald-400">Location</th>
          {hours.map((hour, index) => (
            <th key={index} className="border border-emerald-600 bg-emerald-400">{hour}</th>
          ))}
          <th className="border border-emerald-600 bg-emerald-400">Totals</th>
        </tr>
      </thead>
      <tbody>
        {cookieStands.map((cookieStand, index) => {
          const rowTotal = cookieStand.hourly_sales.reduce((sum, sale) => sum + sale, 0);
          const bgColor = index % 2 === 0 ? 'bg-emerald-200' : 'bg-emerald-300';
          return (
            <tr key={index} className={bgColor}>
              <td className='pl-2 border border-emerald-600 font-bold'>{cookieStand.location}
                <button onClick={() => handleDelete(cookieStand.id)} className=''>
              ...
              {cookieStand.hourly_sales.map((sale, i) => (
                <td key={i} className='pl-2 border border-emerald-600 text-center'>{sale}</td>
              ))}
              <td className='pl-2 border border-emerald-600 text-center'>{rowTotal}</td>
            </tr>
          );
        })}
        <tr>
          <td className="pl-2 border border-emerald-600 text-center font-bold bg-emerald-400">Totals</td>
          {totalRow.map((sum, i) => (
            <td key={i} className='pl-2 border border-emerald-600 text-center bg-emerald-400'>{sum}</td>
          ))}
          <td className='pl-2 border border-emerald-600 text-center bg-emerald-400'>{total}</td>
        </tr>
      </tbody>
    </table>
  ) :
